...
const RESET_CODE_RESPONSE_FALLBACK =
  String(process.env.RESET_CODE_RESPONSE_FALLBACK || 'true').toLowerCase() !== 'false';
...
app.post('/auth/forgot-password', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!RESET_CODE_RESPONSE_FALLBACK) throw emailError;
      logServerError('passwordResetEmailFallbackUsed', emailError, {
        email,
        smtpService: SMTP_SERVICE || null,
        smtpHost: SMTP_HOST || null,
        smtpPort: Number.isInteger(SMTP_PORT) ? SMTP_PORT : null,
      ...
      return res.status(202).json({
        message: 'Reset code delivery failed; use fallback code',
        resetCode: code,
        delivery: 'response',
      });
    }
