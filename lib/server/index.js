...
const readProductLocalizedNames = (source = {}, fallback = {}) => {
  const fallbackName = String(source.name !== undefined ? source.name : fallback.name || '').trim();
  const nameKz = String(
    source.nameKz !== undefined ? source.nameKz : fallback.nameKz || fallbackName
  ).trim();
  const nameRu = String(
    source.nameRu !== undefined ? source.nameRu : fallback.nameRu || fallbackName
  ).trim();
  const nameEn = String(
    source.nameEn !== undefined ? source.nameEn : fallback.nameEn || fallbackName
  ).trim();
  const name = String(source.name !== undefined ? source.name : fallbackName).trim();

  return {
    name: nameKz || name || nameRu || nameEn,
    nameKz,
    nameRu,
    nameEn,
  };
...
const productNamesSearchText = (names) =>
  [names.name, names.nameKz, names.nameRu, names.nameEn]
    .map((value) => String(value || '').trim())
    .filter(Boolean)
    .join(' ');

const toProductResponse = (product) => {
  const names = readProductLocalizedNames(product);
  return {
    id: product._id,
    name: names.name,
    nameKz: names.nameKz,
...
const buildProductSuggestions = async (message, limit = 5) => {
  ...
  if (searchTerms.length > 0) {
    ...
    orFilters.push({ name: regex }, { nameKz: regex }, { nameRu: regex }, { nameEn: regex }, { flowerType: regex });
  }
...
app.post('/products', authRequired, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    const names = readProductLocalizedNames(req.body);
    ...
    const parsedStockCount = Number(stockCount ?? 0);
    if (
      !names.nameKz ||
      !names.nameRu ||
      !names.nameEn ||
    ...
    const tags = resolveProductTags({
      occasionTags,
      recipientTags,
      name: productNamesSearchText(names),
    });
...
app.put('/products/:id', authRequired, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    const hasNameUpdate = ['name', 'nameKz', 'nameRu', 'nameEn'].some(
      (field) => updates[field] !== undefined
    );
    const shouldResolveTags =
      updates.occasionTags !== undefined || updates.recipientTags !== undefined || hasNameUpdate;
    let existing = null;
    if (hasNameUpdate || shouldResolveTags) {
      existing = await Product.findById(req.params.id);
      if (!existing) return res.status(404).json({ message: 'Not found' });
    }
    if (hasNameUpdate) {
      const names = readProductLocalizedNames(updates, existing);
      if (!names.nameKz || !names.nameRu || !names.nameEn) {
        return res.status(400).json({ message: 'All product name translations are required' });
      }
      updates.name = names.name;
      updates.nameKz = names.nameKz;
      updates.nameRu = names.nameRu;
      updates.nameEn = names.nameEn;
    }
    if (shouldResolveTags) {
      const namesForTags = readProductLocalizedNames(updates, existing);
      const tags = resolveProductTags({
        occasionTags: updates.occasionTags,
        recipientTags: updates.recipientTags,
        name: productNamesSearchText(namesForTags),
      });
