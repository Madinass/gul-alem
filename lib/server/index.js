...
const buildVisionCatalog = (products) =>
...
const analyzeImageWithGemini = async ({ imageBase64, mimeType, products }) => {
  ...
  const catalog = buildVisionCatalog(products);
  const prompt = [
    'You are a visual product search engine for the Gul alem flower shop catalog.',
    'Analyze the image and identify the visible product, flower types, colors, arrangement style, and gift items.',
    'Then choose several similar products from the provided catalog. Use only exact catalog ids.',
    ...
    'The terms must be concise English search keywords. Include several catalogMatches when possible.',
...
const getCatalogMatchScores = (analysis) => {
  ...
  matches.forEach((match, index) => {
    ...
    const confidenceScore = Number.isFinite(confidence) ? confidence * 10 : 6;
    scores.set(id, Math.max(scores.get(id) || 0, 18 - index + confidenceScore));
  });
  return scores;
};
...
const rankProductsForVisionAnalysis = (products, analysis, limit = 12) => {
  ...
  const scored = products
    .map((product) => {
      ...
      const score = scoreProductForVision(product, terms) + (catalogMatchScores.get(id) || 0);
  ...
  if (ranked.length > 0 && ranked.length < limit) {
    ...
    const related = products
      .filter((product) => {
        ...
        return (
          product.flowerType === top.flowerType ||
          String(product.category || '') === String(top.category || '')
        );
      })
      .sort((a, b) => {
