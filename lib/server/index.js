...
const GMAIL_FROM = String(process.env.GMAIL_FROM || GMAIL_USER).trim();
...
const sendResetCodeEmail = async ({ email, code }) => {
  ...
  await transporter.sendMail({
    ...
    html: `<p>Your password reset code is <b>${code}</b>.</p><p>It expires in ${RESET_CODE_TTL_MIN} minutes.</p>`,
  });
};
  ...
  try {
    ...
    const cartItem = cartItemId
      ? await CartItem.findOneAndUpdate(
          { _id: cartItemId, user: req.user.userId, itemType: 'custom' },
          { $set: cartData },
          { new: true, runValidators: true }
        )
      : await CartItem.create(cartData);
