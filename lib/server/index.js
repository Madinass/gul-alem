...
const SUPER_ADMIN_EMAIL = process.env.SUPER_ADMIN_EMAIL || 'madinaamandykovna08@gmail.com';
...
const toProductResponse = (product) => ({
  ...
  imageUrl: product.imageUrl || (isExternalImageUrl(product.imagePath) ? product.imagePath : ''),
...
const analyzeImageWithVision = async (imageBase64) => {
  if (typeof fetch !== 'function') {
    const error = new Error('Fetch is not available in this Node.js runtime.');
    error.status = 500;
    throw error;
  }
  if (!GOOGLE_VISION_API_KEY) {
    const error = new Error('Google Vision key missing');
    error.status = 500;
    throw error;
  }
  ...
  if (!response.ok) {
    const error = new Error(payload?.error?.message || `Google Vision failed (${response.status})`);
  ...
  if (annotation.error) {
    const error = new Error(annotation.error.message || 'Google Vision annotation failed');
    error.status = 502;
    throw error;
  }
...
const findProductsByVisionTerms = async (terms, limit = 12) => {
  const products = await Product.find();
  return products
    .map((product) => ({
      product,
      score: scoreProductForVision(product, terms),
    }))
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, limit)
    .map((item) => toProductResponse(item.product));
};
...
app.post('/upload-image', authRequired, requireRole('admin', 'super_admin'), (req, res) => {
...
app.post('/vision/search', async (req, res) => {
  try {
    const imageBase64 = normalizeImageBase64(req.body?.imageBase64 || req.body?.image);
    if (!imageBase64) {
      return res.status(400).json({ message: 'Image is required' });
    }

    const annotation = await analyzeImageWithVision(imageBase64);
    const terms = extractVisionTerms(annotation);
    const parsedLimit = Number.parseInt(req.body?.limit || '12', 10);
    const limit = Number.isFinite(parsedLimit)
      ? Math.min(Math.max(parsedLimit, 1), 30)
      : 12;
    const products = await findProductsByVisionTerms(terms, limit);
    ...
    res.status(error.status || 500).json({
      message: error.message || 'Photo search failed',
    });
  }
});
...
app.post('/orders/custom', authRequired, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (!mongoose.Types.ObjectId.isValid(id) || !Number.isFinite(quantity) || quantity <= 0) {
    ...
    await CustomBouquetItem.bulkWrite(
      customItems.map((item) => ({
        updateOne: {
          filter: { _id: item._id },
          update: { $inc: { stockCount: -requested.get(String(item._id)) } },
        },
      }))
    );
